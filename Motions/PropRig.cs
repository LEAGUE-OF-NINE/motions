...
/// <summary>Owns every unit-anchored prop for one character. Lives on the sidecar object so it
/// inherits the sidecar's lifetime and reads the same director clock that steps sprite frames.</summary>
public class PropRig : MonoBehaviour
{
    ...
    /// <summary>One strike action, resolved once for the whole rig this frame. Everything a strike
    /// needs beyond the slot's own ring position depends only on the action and the clock, so it is
    /// worked out above the slot loop instead of per slot.</summary>
    private class ActiveStrike
    {
        ...
        public bool Arrived;
        /// <summary>The action aimed at the prop's own ring slot, which only the slot loop can
        /// resolve. Arriving there is what un-parks a parked prop.</summary>
    ...
    private int _targetsFrame = -1;

    /// <summary>Names that mean "the target group" rather than one unit.</summary>
    private static bool IsGroupTarget(string to)
        => string.Equals(to, "enemies", StringComparison.OrdinalIgnoreCase)
           || string.Equals(to, "group", StringComparison.OrdinalIgnoreCase);

    private void EnsureTargets()
    {
        if (_targetsFrame == Time.frameCount) return;

        _targetsFrame = Time.frameCount;
        _targets.Clear();
        if (Sync != null) Sync.GetTargetPositions(_targets);
    }
    ...
    private void Initialize()
    {
        ...
        foreach (var entry in entries)
        {
            ...
            _live.Add(new Live
            {
                ...
                Motions = motions,
                MotionKnown = known,
                IsWorld = string.Equals(entry.anchor, "world", StringComparison.OrdinalIgnoreCase),
                IsTarget = isTarget,
    ...
    [HideFromIl2Cpp]
    private bool StrikeTarget(PropAction action, out Vector3 world)
        => ResolveAnchor(action.to, action.offset, out world);

    /// <summary>Where a strike launches from, in world space. Returns false for the default "slot",
    /// which is not a fixed point: it is the slot's own ring position, which only the slot loop
    /// knows. Past that guard it is the same vocabulary <see cref="StrikeTarget"/> reads, through
    /// the same resolver - written twice, "from" lost the group anchors "to" gained and the docs
    /// promised.</summary>
    [HideFromIl2Cpp]
    private bool StrikeOrigin(PropAction action, out Vector3 world)
    {
        world = Vector3.zero;

        if (string.IsNullOrEmpty(action.from)
            || string.Equals(action.from, "slot", StringComparison.OrdinalIgnoreCase))
            return false;

        return ResolveAnchor(action.from, action.fromOffset, out world);
    }
    ...
    /// <para>
    /// Anchors come out of <see cref="EnsureTargets"/>'s frame-cached list rather than walking the
    /// view chain again: that walk is four interop hops plus an Il2Cpp list wrapper allocation per
    /// call, and it ran per strike per frame for an answer already sitting in _targets. The list
    /// skips null targets, so "enemy" is the first *live* target rather than the first entry.
    /// </para></summary>
    [HideFromIl2Cpp]
    private bool ResolveAnchor(string anchor, double[] offsets, out Vector3 world)
    {
        world = Vector3.zero;
        if (Sync == null) return false;

        var offset = new Vector3((float)offsets[0], (float)offsets[1], (float)offsets[2]);

        if (string.Equals(anchor, "self", StringComparison.OrdinalIgnoreCase))
        {
        ...
        if (IsGroupTarget(anchor))
        {
            ...
            for (int i = 0; i < _targets.Count; i++) sum += _targets[i];

            world = sum / _targets.Count + offset;
            return true;
        }

        world = string.Equals(anchor, "center", StringComparison.OrdinalIgnoreCase)
            ? (transform.position + _targets[0]) / 2f + offset
            : _targets[0] + offset;
    ...
    private static bool OwnsSlot(PropAction action, int slot, int actionOrdinal, int count)
    {
        if (string.Equals(action.slot, "all", StringComparison.OrdinalIgnoreCase)) return true;
        if (string.Equals(action.slot, "next", StringComparison.OrdinalIgnoreCase))
            return count > 0 && slot == actionOrdinal % count;
    ...
    private void RefreshCounts()
    {
        ...
        {
            if (roundTurned
                && string.Equals(live.Entry.parkUntil, "round", StringComparison.OrdinalIgnoreCase))
            {
            ...
            if (live.IsWorld)
            {
                bool shouldExist = PropSpec.TargetCount(live.Entry, passes, stack) > 0;

                if (shouldExist && live.WorldHandle == null && !live.Broken)
                {
                    var pos = new Vector3((float)live.Entry.pos[0],
                                          (float)live.Entry.pos[1],
                                          (float)live.Entry.pos[2]);

                    // Caster-relative unless the author asked for absolute scene coordinates.
                    if (!live.Entry.world) pos += transform.position;

                    // Rounds is 0 here: a gated prop lives exactly as long as its gate passes,
                    // so letting the round counter also kill it would give it two owners.
                    live.WorldHandle = PropWorld.Place(AppearanceID, live.Entry, pos, 0,
                                                       out bool ceilingHit, gated: true);

                    // Art or prefab missing (a real failure, already logged by PropWorld/PropLoader)
                    // stops this rig retrying every poll - same flag and reasoning as the grow loop
                    // below. A ceiling hit is transient: space frees as planted instances expire, so
                    // it must not permanently suppress an entry that merely lost a race for slots.
            ...
            // A slot whose GameObject was destroyed from outside - the effect root it hangs off
            // being rebuilt mid-battle - still counts towards the target, so without this the entry
            // sits one instance short for the rest of the battle. PropWorld.Tick self-heals too.
            live.Slots.RemoveAll(s => s.Obj == null);

            int want = PropSpec.TargetCount(live.Entry, passes, stack);

            // A consume: "gate" instance comes back when, and only when, the gate's answer moves.
            // On a keyword entry that is the stack changing; on a fixed-count entry with no keyword
            // it never moves, which is the point of that mode.
            ...
            // A broken entry (missing art, or a throw from Spawn) still has to shrink to zero when
            // its gate drops - only spawning is suppressed, so "count follows the gate" stays true
            // rather than freezing at whatever count existed when it broke.
            while (!live.Broken && live.Slots.Count < want)
            {
                var slot = Spawn(live.Entry, live.Slots.Count, want);
                if (slot == null)
                {
                    // Art or prefab missing, or Spawn threw. Marking Broken (not Entry) stops this
                    // rig retrying every poll without touching the shared cache entry other
                    // characters are still reading.
                    live.Broken = true;
                    break;
                }
                live.Slots.Add(slot);
    ...
    private void ResolveStrikes(Live live, double fraction)
    {
        ...
        for (int a = 0; a < actions.Length; a++)
        {
            ...
            if (string.Equals(action.@do, "plant", StringComparison.OrdinalIgnoreCase)) continue;
            ...
            bool targetIsSlot = string.Equals(action.to, "slot", StringComparison.OrdinalIgnoreCase);
            ...
            bool spread = string.Equals(action.to, "enemies", StringComparison.OrdinalIgnoreCase);
            if (spread)
            {
                EnsureTargets();
            ...
            if (!targetIsSlot && !spread && !StrikeTarget(action, out target)) continue;

            if (_strikeCount == _strikes.Count) _strikes.Add(new ActiveStrike());
            var strike = _strikes[_strikeCount++];

            strike.Action = action;
            strike.Ordinal = mine;
            strike.Target = target;
            strike.TargetIsSlot = targetIsSlot;
            strike.Spread = spread;
            strike.Offset = new Vector3(
                (float)action.offset[0], (float)action.offset[1], (float)action.offset[2]);
            strike.Returning = returning;
            strike.Arrived = fraction >= action.arrive;
            strike.ParkMode = string.IsNullOrEmpty(action.park) ? null : action.park;

            strike.HasOrigin = StrikeOrigin(action, out Vector3 origin);
            strike.Origin = origin;

            // The wind-up only shapes the outbound leg. Coming home there is nothing to draw
            // back from, so the return runs its own progress straight through.
            strike.DrawingBack = false;
            strike.WindUp = action.windUp;

            if (returning)
            {
                strike.Progress = PropSpec.Ease(
            ...
            else
            {
                ...
                if (action.windUp != 0.0)
                {
                    ...
                    strike.DrawingBack = drawingBack;
                    // The draw-back is not eased: easing shapes the throw, and running the same
                    // curve over a pull-back makes the wind-up drift instead of snapping taut.
                    strike.Progress = drawingBack ? leg : PropSpec.Ease(leg, action.ease);
                }
                else
                {
                    strike.Progress = PropSpec.Ease(outbound, action.ease);
                }
            }

            // Arc rides the travelling leg, not the draw-back: the wind-up restarts progress at 0
            // for the throw, so arcing both halves bowed the prop over the pull-back and again over
            // the throw. The pull is straight back along the throw's axis; the throw keeps the bow.
            strike.Arc = strike.DrawingBack ? 0.0 : PropSpec.ArcOffset(action.arc, strike.Progress);

            // Consumed on arrival. Hidden rather than destroyed: props cannot spend buff stacks,
            // so the gate would respawn it within a frame. A parking strike is exempt - park and
            // consume are both "no returnAt" endings, and the hide rule keys off exactly the field
            // park leaves unset, so without this check every parked prop vanishes on arrival.
            strike.Hidden = fraction >= action.arrive
                            && action.returnAt < 0
                            && string.IsNullOrEmpty(action.park);
            strike.ConsumeToGate = strike.Hidden
                && string.Equals(action.consume, "gate", StringComparison.OrdinalIgnoreCase);

            strike.HasSpin = action.spin != 0;
            strike.SpinDegrees = action.spin * fraction * _timebase;

            // spin beats face: a rate and an orientation rule cannot both own the rotation, and
            // spin is the more explicit of the two.
            strike.HasFace = action.face && !strike.HasSpin;
        }
    }
                                   ...
                                   out double faceDegrees)
    {
        ...
        if (slot.ParkTargetIndex >= 0)
        {
            ...
            if (slot.ParkTargetIndex < _targets.Count)
            {
                var offset = slot.ParkAction.offset;
                centre = _targets[slot.ParkTargetIndex]
        ...
        {
            if (StrikeTarget(slot.ParkAction, out Vector3 resolved)) centre = resolved;
        ...
        bool holds = string.Equals(mode, "hold", StringComparison.OrdinalIgnoreCase);
    ...
    private void Place(Live live, float now, double fraction)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            bool struck = false;
            bool hidden = false;
            // The entry's ambient spin, 0 for an entry that has none - which is also what returns
            // an unspun prop to its original facing. A live strike overrides it below, as does a
            // parked slot that landed with face.
            double spinDegrees = live.Entry.spin * now;
            Vector3 at = default;

            if (_strikeCount > 0)
            {
                // Resolved lazily, at most once per slot: TransformPoint is an interop call, and
                // most slots aren't owned by any live strike on a given frame - a 16-slot ring with
                // one "next" strike has no business paying for 16 of these.
                bool haveHome = false;
                Vector3 worldHome = default;
                Vector3 slotHome = default;

                // Last owning action wins outright - position, hidden and spin together. Latching
                // hidden across actions left a consume-then-return pair lerping an invisible prop.
                for (int s = 0; s < _strikeCount; s++)
                {
                    var strike = _strikes[s];
                    if (!OwnsSlot(strike.Action, i, strike.Ordinal, count)) continue;

                    // A parking strike hands the slot over once parked; from then on the parked
                    // branch owns the position. Without this the strike keeps pinning the prop to
                    // the exact target while the formation tries to orbit it.
                    ...
                    if (!haveHome)
                    {
                        ...
                        // A parked prop launches from where it stands, not from the ring slot it
                        // left - that is what lets one motion move it out and a later one throw it
                        // from there. Resolved from the park, never read off the transform: this
                        // loop writes that transform below, so reading it would feed each frame's
                        // position into the next frame's launch point - flight collapses into a few
                        // frames and a return leg pins the prop to its target instead of lerping
                        // home. Same call the unstruck parked branch makes.
                        slotHome = slot.ParkAction != null
                            ? ParkedPosition(live, slot, i, count, now, out _)
                            : worldHome;
                        haveHome = true;
                    }

                    // "slot" launches from wherever this slot is parked, which is why the origin is
                    // resolved here rather than with the rest of the strike.
                    Vector3 origin = strike.HasOrigin ? strike.Origin : slotHome;

                    // A recall aims at the ring slot itself, and a spread deals this slot its own
                    // enemy - both are things only this loop knows.
                    Vector3 strikeTarget = strike.TargetIsSlot ? worldHome
                        : strike.Spread ? _targets[i % _targets.Count] + strike.Offset
                        : strike.Target;

                    Vector3 toTarget = strikeTarget - origin;

                    if (strike.DrawingBack)
                    {
                        // Away from the target along the same axis it will travel, so the throw
                        // reads as one motion reversed rather than a detour.
                        Vector3 back = toTarget.sqrMagnitude > 0f
                            ? -toTarget.normalized * (float)strike.WindUp
                            : Vector3.zero;
                        at = Vector3.Lerp(origin, origin + back, (float)strike.Progress);
                    }
                    else if (strike.Returning)
                    {
                        // Home is the parked spot when there is one: park is sticky, so an ordinary
                        // strike borrows a parked prop and gives it back rather than un-parking it.
                        at = Vector3.Lerp(strikeTarget, slotHome, (float)strike.Progress);
                    }
                    else
                    {
                        // Past the draw-back, the throw starts from where it drew back to.
                        Vector3 launch = origin;
                        if (strike.WindUp != 0.0 && toTarget.sqrMagnitude > 0f)
                            launch = origin - toTarget.normalized * (float)strike.WindUp;

                        at = Vector3.Lerp(launch, strikeTarget, (float)strike.Progress);
                    }

                    if (strike.Arc != 0.0) at += new Vector3(0f, (float)strike.Arc, 0f);

                    hidden = strike.Hidden;
                    spinDegrees = strike.HasSpin ? strike.SpinDegrees : live.Entry.spin * now;

                    // Faced off the leg's own axis, not the frame's movement delta: the delta is
                    // zero on the frame a leg starts, and a knife flipping to 0 degrees for one
                    // frame at launch is more visible than any accuracy that buys.
                    if (strike.HasFace)
                    {
                        Vector3 facing = strike.Returning ? slotHome - strikeTarget : toTarget;
                        spinDegrees = PropSpec.FaceDegrees(facing.x, facing.y);
                    }
                    ...
                    if (strike.Arrived)
                    {
                        if (strike.ParkMode != null)
                        {
                            ...
                            slot.ParkTargetIndex = strike.Spread ? i % _targets.Count : -1;
                            // Frozen here so "hold" keeps the angle it arrived wearing, not one
                            // picked when the formation is first drawn.
                            slot.ParkT = now;
                            slot.ParkFaced = strike.HasFace;
                            slot.ParkDegrees = spinDegrees;
                        }
                        // Coming home is what ends a park. Any other target leaves it parked, so
                        // an ordinary strike borrows a parked prop instead of stranding it.
                        else if (strike.TargetIsSlot) slot.Unpark();
                    }
                }
            }
            ...
            else if (struck) slot.Obj.transform.position = at;
    ...
    /// <summary>Fires plant actions whose time was crossed since the previous frame. Edge-triggered
    /// rather than time-derived like a strike, because a plant is a one-off: evaluating it
    /// continuously would place a new instance every frame for the rest of the motion.</summary>
    [HideFromIl2Cpp]
    private void TickPlants(Live live, double fraction, bool motionRestarted)
    {
        var actions = live.Entry.actions;
        ...
        for (int a = 0; a < actions.Length; a++)
        {
            ...
            if (!(fraction >= action.at && previous < action.at)) continue;

            Vector3 where;
            if (live.Entry.world)
            {
                where = new Vector3((float)action.pos[0], (float)action.pos[1], (float)action.pos[2]);
            }
            else if (StrikeTarget(action, out Vector3 anchor))
            {
                where = anchor + new Vector3((float)action.pos[0], (float)action.pos[1], (float)action.pos[2]);
            }
            else
            {
            ...
            // Discarded here: a plant that hit the ceiling or failed to build gives TickPlants
            // nothing to do differently, and PropWorld already logged whichever it was.
            PropWorld.Place(AppearanceID, live.Entry, where, action.rounds, out _);
        }
    }
    ...
    private void Step()
    {
        ...
        foreach (var live in _live)
        {
            ...
            if (fraction >= 0) TickPlants(live, fraction, motionRestarted);
        }
